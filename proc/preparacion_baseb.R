...
# Resumen ejectuvo: Este script de procesamiento contiene el código que permite elaborar la Base de Datos B, la cual se utiliza para análisis secundarios y que no necesariamente estarán reportadas en el artículo final.

# La Base de Datos B incluye las variables de autoritarismo (AC) por separado y las variables de confianza en las instituciones (CONF) como indices.
...
datos_proc_b$ac_destino <- car::recode(datos_proc_b$ac_destino,
                                       as.numeric = T,
                                       "1 = 5;
   2 = 4;
   5 = 3;
...
frq(datos_proc_b$ac_destino) # Recodificación

## ac_mano_dura

datos_proc_b$ac_mano_dura <- car::recode(datos_proc_b$ac_mano_dura,
                                         as.numeric = T,
                                         "1 = 5;
   2 = 4;
   5 = 3;
...
frq(datos_proc_b$ac_mano_dura) # Recodificación

## ac_obedecer_padres

datos_proc_b$ac_obedecer_padres <- car::recode(datos_proc_b$ac_obedecer_padres,
                                               as.numeric = T,
                                               "1 = 5;
   2 = 4;
   5 = 3;
...
frq(datos_proc_b$ac_obedecer_padres) # Recodificación

## ac_obedecer_autoridades_gob

datos_proc_b$ac_obedecer_autoridad_gob <- car::recode(datos_proc_b$ac_obedecer_autoridad_gob,
                                                      as.numeric = T,
                                                      "1 = 5;
   2 = 4;
   5 = 3;
...
frq(datos_proc_b$ac_obedecer_autoridad_gob) # Recodificación


# Transformación de variables cuyas categorías van del 1 al 10 a variables
#numéricas

datos_proc_b <- datos_proc_b %>%
...
mutate_at(vars(starts_with("dinero")),
  ...
  # # de las variables que comienzan por ac
  mutate_at(vars(starts_with("ac")),
            ~(factor(.,levels = c(1,2,3,4,5), labels = c("Muy en desacuerdo",
                                  "En desacuerdo",
...
datos_proc_b$ind_perc_pas <- sjlabelled::set_label(datos_proc_b$ind_perc_pas, label = "Indice percepción situación en el pasado") # Etiquetar nueva variable
...
datos_proc_b$ind_perc_fut <- sjlabelled::set_label(datos_proc_b$ind_perc_fut, label = "Indice percepción situación en el futuro") # Etiquetar nueva variable
...
datos_proc_b$ind_perc <- sjlabelled::set_label(datos_proc_b$ind_perc, label = "Indice percepción situación actual") # Etiquetar nueva variable
...
datos_proc_b$ind_conf_inst <- sjlabelled::set_label(datos_proc_b$ind_conf_inst, label = "Indice confianza en instituciones") # Etiquetar nueva variable


# 3. 7. Analizar los descriptivos de todas las variables -------------------

df_gen_b<- dfSummary(datos_proc_b,
                   plain.ascii = FALSE,
                   style = "grid",
                   tmp.img.dir = "/tmp",
                   graph.magnif = TRUE,
...
df_gen$Variable <- NULL # delete variable column
summarytools::view(df_gen, file = "output/tables/df_gen_b.html") # Ver tabla en un archivo HTML

# Efectivamente horas_trabajo es la única variable con más de mil NA
# seguro_social es la segunda variable con más NA

datos_proc_b %>% VIM::aggr() # Ver patrones de NA

table1::table1(~horas_trabajo | pais, data = datos_proc_b)
...
save(datos_proc_b, file = "input/data/datos_proc_b.RData") 

saveRDS(datos_proc_b, file = "input/data/datos_proc_b.rds") 
saveRDS(costa_rica_b, file = "input/data/costa_rica_b.rds") 
saveRDS(nicaragua_b, file = "input/data/nicaragua_b.rds") 
saveRDS(el_salvador_b, file = "input/data/el_salvador_b.rds") 
saveRDS(guatemala_b, file = "input/data/guatemala_b.rds") 
saveRDS(honduras_b, file = "input/data/honduras_b.rds") 