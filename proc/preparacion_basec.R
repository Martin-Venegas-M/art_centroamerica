...
# Resumen ejectuvo: Este script de procesamiento contiene el código que permite elaborar la Base de Datos C, la cual se utiliza para análisis secundarios y que no necesariamente estarán reportadas en el artículo final.

# La Base de Datos C incluye las variables de autoritarismo (AC) como indice y las variables de confianza en las instituciones (CONF) por separado.
...
datos_proc_c$ac_destino <- car::recode(datos_proc_c$ac_destino,
                                       as.numeric = T,
                                       "1 = 5;
   2 = 4;
   5 = 3;
...
frq(datos_proc_c$ac_destino) # Recodificación

## ac_mano_dura

datos_proc_c$ac_mano_dura <- car::recode(datos_proc_c$ac_mano_dura,
                                         as.numeric = T,
                                         "1 = 5;
   2 = 4;
   5 = 3;
...
frq(datos_proc_c$ac_mano_dura) # Recodificación

## ac_obedecer_padres

datos_proc_c$ac_obedecer_padres <- car::recode(datos_proc_c$ac_obedecer_padres,
                                               as.numeric = T,
                                               "1 = 5;
   2 = 4;
   5 = 3;
...
frq(datos_proc_c$ac_obedecer_padres) # Recodificación

## ac_obedecer_autoridades_gob

datos_proc_c$ac_obedecer_autoridad_gob <- car::recode(datos_proc_c$ac_obedecer_autoridad_gob,
                                                      as.numeric = T,
                                                      "1 = 5;
   2 = 4;
   5 = 3;
...
frq(datos_proc_c$ac_obedecer_autoridad_gob) # Recodificación


# Transformación de variables cuyas categorías van del 1 al 10 a variables
#numéricas

datos_proc_c <- datos_proc_c %>%
...
mutate_at(vars(starts_with("dinero")),
  ...
  #de las variables que comienzan por conf
  mutate_at(vars(starts_with("conf")),
            ~(factor(.,levels = c(1,2,3), labels = c("Baja",
                                  "Regular",
                                  "Alta"))))
...
datos_proc_c$ind_perc_pas <- sjlabelled::set_label(datos_proc_c$ind_perc_pas, label = "Indice percepción situación en el pasado") # Etiquetar nueva variable
...
datos_proc_c$ind_perc_fut <- sjlabelled::set_label(datos_proc_c$ind_perc_fut, label = "Indice percepción situación en el futuro") # Etiquetar nueva variable
...
datos_proc_c$ind_perc <- sjlabelled::set_label(datos_proc_c$ind_perc, label = "Indice percepción situación actual") # Etiquetar nueva variable
...
datos_proc_c$ind_aut <- sjlabelled::set_label(datos_proc_c$ind_aut, label = "Indice autoritarismo") # Etiquetar nueva variable


# 3. 7. Analizar los descriptivos de todas las variables -------------------

df_gen_c<- dfSummary(datos_proc_c,
                   plain.ascii = FALSE,
                   style = "grid",
                   tmp.img.dir = "/tmp",
                   graph.magnif = TRUE,
...
df_gen_c$Variable <- NULL # delete variable column
summarytools::view(df_gen_c, file = "output/tables/df_gen_c.html") # Ver tabla en un archivo HTML

# Efectivamente horas_trabajo es la única variable con más de mil NA
# seguro_social es la segunda variable con más NA

datos_proc_c %>% VIM::aggr() # Ver patrones de NA

table1::table1(~horas_trabajo | pais, data = datos_proc_c)
...
save(datos_proc_c, file = "input/data/datos_proc_c.RData") 

saveRDS(datos_proc_c, file = "input/data/datos_proc_c.rds") 
saveRDS(costa_rica_c, file = "input/data/costa_rica_c.rds") 
saveRDS(nicaragua_c, file = "input/data/nicaragua_c.rds") 
saveRDS(el_salvador_c, file = "input/data/el_salvador_c.rds") 
saveRDS(guatemala_c, file = "input/data/guatemala_c.rds") 
saveRDS(honduras_c, file = "input/data/honduras_c.rds") 
